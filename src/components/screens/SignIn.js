...
const SignIn = props => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        if (profile) {
          ...
          Alert.alert(
            'Error while Sign In',
            'There was an error while signing you in, please try again',
          );
        }
      } else {
        console.log(
          `User couldn't be signed in: ${JSON.stringify(response.data)}`,
        );
        Alert.alert(
  ...
  const login = () => {
    ...
    if (!checkValid) {
      ...
      Alert.alert(
        'Invalid Password',
        'Password must be of minimum eight characters, at least one letter, one number and one special character',
      );
    } else {
      userSignIn();
    }
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
              <View>
                ...
                <View style={styles.navigate}>
                  <Text style={styles.navigateText} onPress={navigate}>
                    New here? Sign Up
                  </Text>
                </View>
              </View>
